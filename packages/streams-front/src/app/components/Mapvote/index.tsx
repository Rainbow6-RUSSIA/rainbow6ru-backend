    ...
    return (
      <FadeInDown className={style.container} pose={isVisible ? 'out' : 'in'}>
        <div className={style.logs}>
          <Log votes={votes.filter((v, i) => v.teamId === match.teams[0].id && v.type !== 'decider')} team={match.teams[0]}/>
          {match.votes[match.poolCache.length - 1] ? <Decider vote={match.votes[match.poolCache.length - 1]} teams={match.teams}/> : null}
          <Log votes={votes.filter((v, i) => v.teamId === match.teams[1].id && v.type !== 'decider')} team={match.teams[1]}/>
        </div>
