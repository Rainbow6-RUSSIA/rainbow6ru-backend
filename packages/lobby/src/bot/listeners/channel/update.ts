...
export default class Update extends Listener {
    ...
    public exec = async (oldChannel: DMChannel | GuildChannel, newChannel: DMChannel | GuildChannel) => {
        if (
            ...
            const settigns = Object.values(room.LS.guild.lobbySettings).find(
                (s) => s.voiceCategory === newChannel.parentID,
            );
            if (settigns) {
                ...
                await Promise.all([room.LS.syncChannels(), newRoom.LS.syncChannels()]); // refactor to replace with last voice
                await Promise.all([room.LS.updateFastAppeal(), newRoom.LS.updateFastAppeal()]);
