...
export default class Stats extends Command {
    ...
    public exec = async (message: Message, args: IArgs) => {
        ...
        return message.reply(
            `всего уникальных пользователей пользователей прошло через лобби с момента их загрузок: \`${
                ...
                Object.keys(dbGuild.lobbySettings)
                    ...
                    .map(
                        (LS) =>
                            `Категория \`${LS.settings.type}\` - \`${
                                LS.uniqueUsers.size
                            }\` уникальных пользователей за \`${humanizeDuration(Date.now() - LS.loadedAt.valueOf(), {
                                conjunction: ' и ',
                                language: 'ru',
                                round: true,
                            })}\``,
