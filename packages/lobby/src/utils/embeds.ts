...
export default {
    appealMsg: (lobby: LSRoom): MessageOptions => {
        const embed = new MessageEmbed()
            ...
            .setDescription(
                lobby.members
                    ...
                    .map(
                        (m) =>
                            (lobby.dcLeader.id === m.id ? '\\👑 ' : '') +
                            (!m.platform.PC ? '\\🎮' : '') +
                            `<@${m.id}> (${bot.emojis.resolve(RANK_BADGES[m.rank])} **${Util.escapeMarkdown(
                                m.nickname,
                            )}** - [${HF_PLATFORM[Object.entries(m.platform).find((e) => e[1])[0]]}](${m.toString()})${(
                                ' | ' + m.region
                            )
                                .replace(/.+emea/g, '')
                                .replace('ncsa', '🌎')
                                .replace('apac', '🌏')})` +
                            (m.verificationLevel >= VERIFICATION_LEVEL.QR
                                ? ` ${bot.emojis.resolve(VERIFIED_BADGE)}`
                                : ''),
                    )
                    .join('\n') + (lobby.description ? `\n▫${lobby.description}` : ''),
            )
            .setFooter(
                `В игре ники Uplay отличаются? Cообщите администрации со скрином таба. С вами ненадежный игрок! • S: ${
                    IS[lobby.status]
                } ID: ${lobby.id}`,
                'https://i.imgur.com/sDOEWMV.png',
            )
            .setThumbnail(
                `${ENV.LOBBY_PREVIEW_URL}/${lobby.id}/preview?a${lobby.minRank}.${lobby.maxRank}.${
        ...
        if (lobby.hardplay) {
            embed.addField(
                `Режим "HardPlay\\${EmojiButtons.HARDPLAY}"`,
                `Минимальный ранг для входа: \`${
                    RANKS[lobby.guild.rankRoles.findIndex((r) => lobby.guild.rankRoles[lobby.minRank] === r)]
                }\``,
            );
        }
        ...
        if ([IS.NEWCOMER, IS.NEWCOMER_SEARCH].includes(lobby.status)) {
            embed.addField(
                'Режим "Новичок"',
                'Опытным игрокам лучше найти другую комнату, чтобы избежать конфликтов и поражений.',
            );
        }
        ...
        if (lobby.joinAllowed) {
            ...
            !lobby.close &&
            lobby.dcMembers.size < lobby.dcChannel.userLimit &&
            currentlyPlaying.includes(lobby.status)
        ) {
    ...
    fastAppeal: async (LS: LobbyStore): Promise<MessageOptions> => {
        ...
        const embed = new MessageEmbed()
            ...
            .setDescription(
                `Канал поиска: ${LS.lfgChannel}\n` +
                    `Всего лобби: \`${LS.rooms.filter((v) => Boolean(v.dcMembers.size)).size}\`\n` +
                    `Ищут игрока: \`${
                        LS.rooms.filter((l) => Boolean(l.dcMembers.size) && l.appealMessage && l.joinAllowed).size ||
                        (LS.rooms.filter((l) => Boolean(l.dcMembers.size) && Boolean(l.appealMessage)).size
                            ? 'все комнаты укомплектованы!'
                            : 0)
                    }\`\n` +
                    `Присоединиться к новой комнате: ${await (
                        LS.rooms.filter((r) => !r.dcMembers.size).last() || LS.rooms.last()
                    ).initInvite()} 👈`,
            );
        LS.rooms
            ...
            .map((lobby) =>
                embed.addField(
                    ...
                    (lobby.hardplay
                        ? `HardPlay\\${EmojiButtons.HARDPLAY}: только \`${
                              RANKS[lobby.guild.rankRoles.findIndex((r) => lobby.guild.rankRoles[lobby.minRank] === r)]
                          }\` и выше\n`
                        : `Ранг: ${
                              lobby.minRank === lobby.maxRank
                                  ? lobby.maxRank === 0
                                      ? '`любой`'
                                      : `от \`${
                                            RANKS[extractBorders([lobby.minRank, lobby.maxRank])[0]]
                                        }\`${bot.emojis.resolve(
                                            RANK_BADGES[extractBorders([lobby.minRank, lobby.maxRank])[0]],
                                        )} до \`${
                                            RANKS[extractBorders([lobby.minRank, lobby.maxRank])[1]]
                                        }\`${bot.emojis.resolve(
                                            RANK_BADGES[extractBorders([lobby.minRank, lobby.maxRank])[1]],
                                        )}`
                                  : `от \`${RANKS[lobby.minRank]}\`${bot.emojis.resolve(
                                        RANK_BADGES[lobby.minRank],
                                    )} до \`${RANKS[lobby.maxRank]}\`${bot.emojis.resolve(RANK_BADGES[lobby.maxRank])}`
                          }\n`) +
                        ([IS.NEWCOMER, IS.NEWCOMER_SEARCH].includes(lobby.status)
                            ? 'Новичок: не выше `50` уровня доступа\n'
                            : '') +
    ...
    appealMsgPremium: (member: GuildMember, description: string, invite: string): MessageOptions => ({
        embed: new MessageEmbed()
            .setAuthor(
                `${member.user.tag} ищет +${
                    member.voice.channel.userLimit - member.voice.channel.members.size
                } в свою уютную комнату | ${member.voice.channel.name}`,
            ...
            .setFooter(
                `Хотите так же? Обратитесь в ЛС Сервера или к ${member.guild.owner.user.tag} с рублями из маминого кошелька 💵, или активируйте Nitro Boost 💜.`,
                'https://cdn.discordapp.com/emojis/414787874374942721.png?v=1',
            )
...
const modeSelector = (lobby: LSRoom) => {
    const slot = lobby.joinAllowed ? ` | +${lobby.dcChannel.userLimit - lobby.dcMembers.size} слот(-а)` : '';
    switch (lobby.status) {
