...
export default function PartyCommand(skipLeadership = false) {
    return <T extends Command, K extends keyof T>(
        ...
        propertyDesciptor.value = async function (message: Message, args) {
            ...
            if (!room) {
                return DMReply(message, 'Команды пати доступны только для голосовых каналов игровых категорий');
            }
            if (room.LS.lfgChannel.id !== message.channel.id) {
                return DMReply(
                    message,
                    'Команды пати доступны только в соответствующем канале поиска игровой категории!',
                );
            } else {
                if (
                    !message.member.permissions.has('MANAGE_ROLES') &&
                    !skipLeadership &&
                    room.dcLeader?.id !== message.author.id
                ) {
