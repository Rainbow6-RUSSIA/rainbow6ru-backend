...
export async function createLobbyPreview(n: number, m: number, k = 0) {
    if (!(Number.isInteger(n) && Number.isInteger(m) && Number.isInteger(k))) {
        return;
    }
    ...
    const images = await Promise.all([
        Canvas.loadImage(__dirname + `/../../assets/ranks/${n}.png`),
        Canvas.loadImage(__dirname + `/../../assets/ranks/${m}.png`),
    ]);
