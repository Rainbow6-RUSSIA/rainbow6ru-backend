...
export default class Stats extends Command {
    ...
    public exec = async (message: Message, args: IArgs) => {
        switch (true) {
            ...
            case args.type === 'voice': {
                ...
                return message.reply(
                    `всего пользователей в голосовых каналах: \`${guild.channels
                        ...
                        Object.values(dbGuild.lobbySettings)
                            ...
                            .map((ent) => [
                                ent[0],
                                (guild.channels.get(ent[1]) as CategoryChannel).children
                                    .filter((ch) => ch.type === 'voice')
                                    .reduce((acc, val: VoiceChannel) => acc + val.members.size, 0),
                            ])
            ...
            case !args.type: {
                ...
                const dbGuild = await Guild.findByPk(guild.id);
                const rankRoles = guild.roles
                    .filter((r) => dbGuild.rankRoles.includes(r.id))
                    .array()
                    .sort((a, b) => dbGuild.rankRoles.indexOf(b.id) - dbGuild.rankRoles.indexOf(a.id));
                const Users = await User.findAll({
                    attributes: ['id', 'rank', 'verificationLevel', 'requiredVerification', 'inactive'],
                });
                return message.reply(
    ...
    public static appendix = (arr: User[]) =>
        ...
        `**Верифицированных участников**: \`${
            arr.filter((u) => u.verificationLevel >= VERIFICATION_LEVEL.QR).length
        }\`\n` +
