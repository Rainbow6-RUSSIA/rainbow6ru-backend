...
export default class Locate extends Command {
    ...
    public exec = async (message: Message, args: IArgs) => {
        ...
        return message.reply(
            `пользователь ${user} \`${user.tag}\`: ${
                !member
                    ? 'не на сервере'
                    : !member.voice.channel
                    ? 'не в голосовом канале'
                    : `находится в <#${member.voice.channel.id}> ${await (async () => {
                          try {
                              const inv = await member.voice.channel.createInvite({
                                  maxUses: 5,
                                  maxAge: 600,
                                  reason: 'поиск по голосовым каналам',
                              });
                              return inv.url;
                          } catch (error) {
                              /* */
