        ...
        dbUser.isInVerification
            ? `*В целях безопасности требуется подтверждение аккаунта Uplay.`
                ...
                + (status === UpdateStatus.DM_CLOSED
                    ? (isAdmin ? 'ЛС закрыто.' : `Откройте личные сообщения для ${this.client.user} и повторите попытку.`)
                    : (isAdmin ? ' Инструкции высланы пользователю в ЛС.' : ` Следуйте инструкциям, отправленным в ЛС c ${this.client.user}.`))
                + `*`
            : ''
}
