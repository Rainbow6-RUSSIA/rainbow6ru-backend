...
export default async (message: Message, phrase: string): Promise<IUbiBoundType | Flag> => {
    ...
    try {
        ...
        if (!phrase.toLowerCase().startsWith('ubi_') && !phrase.toLowerCase().endsWith('_ubi')) {
            switch (true) {
                case /^[a-zA-Z][\w-._]{2,16}$/.test(phrase): return await fetchProfiles(platforms, phrase); // await errors

                case /^[a-zA-Z][\w-._ ]{2,16}$/.test(phrase): return await fetchProfiles(platforms.filter(p => p === PLATFORM.XBOX), phrase); // await errors
            }
        }
    } catch (error) {
        return error;
