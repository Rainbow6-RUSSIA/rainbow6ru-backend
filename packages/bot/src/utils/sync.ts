  ...
  public static async sendQrRequest(dbGuild: G, dbUser: U, member: GuildMember) {
    ...
    try {
      await member.send(
        ...
        + `Для этого нужно установить прикрепленное ниже изображение с QR-кодом на аватар в настройках Uplay и после смены ввести команду \`$verify\` в ответ на **это** сообщение.\n`
        + `\n`
        ...
        + `Рекомендуем нажать на изображение и скачать с помощью кнопки "Открыть оригинал" для исключения ошибок.\n`
        + `**После верификации аватар можно сменить назад.** (текущий можно сохранить по ссылке <https://ubisoft-avatars.akamaized.net/${dbUser.genome}/default_256_256.png>)`,
