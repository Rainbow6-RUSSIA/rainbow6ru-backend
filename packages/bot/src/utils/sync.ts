...
export default class Sync {
    public static async updateNicknames(platform: PLATFORM) {
        ...
        const res = await r6.api.getCurrentName(
            platform,
            dbUsers.map((u) => u.genome),
        );
    ...
    public static async updateRank(platform: PLATFORM) {
        ...
        if (!dbUsers.length || dbUsers[0].rankUpdatedAt.valueOf() + 50 * parseInt(ENV.COOLDOWN) > Date.now()) {
            return [];
        }
        const res = await r6.api.getRank(
            platform,
            dbUsers.map((u) => u.genome),
        );
        if (!Object.keys(res).length) {
        ...
        await Promise.all(
            dbUsers.map((u) => {
                if (res[u.genome]) {
                    ...
                    // }
                    u.rank = u.region
                        ? res[u.genome][u.region].rank
                        : Math.max(res[u.genome].apac.rank, res[u.genome].ncsa.rank, res[u.genome].emea.rank);
                }
    ...
    public static async sendQrRequest(dbGuild: G, dbUser: U, member: GuildMember) {
        ...
        await (await guild.members.fetch(dbUser.id)).roles.remove(
            [...dbGuild.rankRoles.filter(Boolean), ...Object.values(dbGuild.platformRoles).filter(Boolean)],
            'запрос верификации',
        );
    ...
    public static async updateMember(dbGuild: G, dbUser: U) {
        if (!dbGuild || !dbUser) {
            return UpdateStatus.INCORRECT_CALL;
        }
        if (!dbGuild.premium) {
            return UpdateStatus.GUILD_NONPREMIUM;
        }
        const guild = bot.guilds.get(dbGuild.id);
        if (!guild.available) {
            return UpdateStatus.GUILD_UNAVAILABLE;
        }
        ...
        if (dbUser.syncNickname) {
            try {
                await member.setNickname(
                    `${dbUser.teamId ? (await Team.findByPk(dbUser.teamId)).shortName + '.' : ''}${dbUser.nickname}${
        ...
        const platformRolesToApply = Object.entries(dbGuild.platformRoles)
            .filter((k) => dbUser.platform[k[0]])
            .map((k) => k[1]);
        ...
        if (currentRankRoles.length > 1) {
            ...
            if (
                (dbUser.rank > currentRank || currentRank < dbGuild.fixAfter || dbUser.rank === 0) &&
                currentRankRoles[0] !== dbGuild.rankRoles[dbUser.rank]
            ) {
                finalRoles = [
                    ...new Set(
                        [...finalRoles, dbGuild.rankRoles[dbUser.rank]].filter(
                            (r) => r !== dbGuild.rankRoles[currentRank],
                        ),
                    ),
                ];
    ...
    public static async updateRoles() {
        ...
        const usersAtPlatforms = await Promise.all(
            $enum(PLATFORM)
                .getValues()
                .map((p) => Sync.updateRank(p)),
        );
        ...
        await Promise.all(
            guilds
                .map((g) =>
                    users.filter((u) => bot.guilds.get(g.id).members.has(u.id)).map((u) => Sync.updateMember(g, u)),
                )
