...
export default {
  appealMsg: async (lobby: Lobby): Promise<MessageOptions> => ({
    embed: {
      ...
      description: (lobby.members.sort((a, b) => b.rank - a.rank).map((m) => `<@${m.id}> (\`${m.nickname}\` - [[r6tab](${ONLINE_TRACKER}${m.genome})]) ${m.verificationLevel >= VERIFICATION_LEVEL.QR ? ENV.VERIFIED_BADGE : ''}`).join('\n'))
        + (lobby.description
          ? `\n▫${lobby.description}`
          : ''),
